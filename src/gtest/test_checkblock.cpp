...
TEST(CheckBlock, VersionTooLow) {
    ...
    EXPECT_FALSE(CheckBlock(&futureblock,0,0,block, state, verifier, false, false, true));
}
...
TEST(CheckBlock, BlockSproutRejectsBadVersion) {
    ...
    EXPECT_FALSE(CheckBlock(block, state, verifier, false, false, true));
}
...
protected:
    ...
    void ExpectValidBlockFromTx(const CTransaction& tx) {
        ...
        MockCValidationState state;
        EXPECT_TRUE(ContextualCheckBlock(block, state, &indexPrev, true));
    }
    ...
    void ExpectInvalidBlockFromTx(const CTransaction& tx, int level, std::string reason) {
        ...
        EXPECT_FALSE(ContextualCheckBlock(block, state, &indexPrev, true));
    }
...
TEST_F(ContextualCheckBlockTest, BadCoinbaseHeight) {
    ...
    MockCValidationState state;
    EXPECT_TRUE(ContextualCheckBlock(block, state, NULL, true));
